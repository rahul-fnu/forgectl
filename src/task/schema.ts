import { z } from "zod";

const duration = z.string().regex(/^\d+(s|m|h|d)$/, "Must be a duration like 30s, 5m, 1h, 7d");

export const AcceptanceCriterionSchema = z.object({
  run: z.string().optional(),
  assert: z.string().optional(),
  description: z.string().optional(),
}).refine(
  (data) => data.run !== undefined || data.assert !== undefined || data.description !== undefined,
  { message: "At least one of run, assert, or description is required" },
);

export const DecompositionConfigSchema = z.object({
  strategy: z.enum(["auto", "manual", "none"]).default("auto"),
  max_depth: z.number().int().min(1).max(5).optional(),
});

export const EffortConfigSchema = z.object({
  max_turns: z.number().int().min(1).max(200).optional(),
  max_review_rounds: z.number().int().min(0).max(5).optional(),
  timeout: duration.optional(),
});

export const TaskBudgetSchema = z.object({
  max_cost_usd: z.number().positive().optional(),
  max_tokens: z.number().int().positive().optional(),
}).optional();

export const TaskSpecSchema = z.object({
  id: z.string().regex(/^[a-z0-9][a-z0-9-]*$/, "ID must be lowercase alphanumeric with hyphens, starting with a letter or digit"),
  title: z.string().min(1).max(200),
  description: z.string().optional(),
  context: z.object({
    files: z.array(z.string()).min(1),
    docs: z.array(z.string()).optional(),
    modules: z.array(z.string()).optional(),
    related_tasks: z.array(z.string()).optional(),
  }),
  constraints: z.array(z.string()).default([]),
  acceptance: z.array(AcceptanceCriterionSchema).min(1),
  decomposition: DecompositionConfigSchema.default({ strategy: "auto" }),
  effort: EffortConfigSchema.default({}),
  budget: TaskBudgetSchema,
  metadata: z.record(z.string()).optional(),
});
