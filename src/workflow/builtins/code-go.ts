...
Your workspace is at /workspace containing the Go project repository.
...
- After modifying types or interfaces → run \`go vet\`
...
- Never suppress errors by weakening types (adding \`interface{}\` where a concrete type exists), disabling lint rules, or skipping tests.
